...
function commandHandler (Packet: any[], Buffer2: number, serialN: number) {
...
function FBV1 (data: string) {
    
hash2 = sec.FNV1aHash(data)
    return hash2
}
function packetHandler (serialN2: number, Signal_strength: number, Data: string, buffer: number) {
    Packet2 = []
...
function packetHelper (From: number, Mode: number, To: number, data2: string, buffer2: number) {
...
function config () {
    ...
    serial.writeLine("Check confirmed")
}
...
function ClientHelper (SRC: number, mode: number) {
    let index3: number;
let index: number;
if (mode == 0) {
        serial.writeLine("definition Setup")
...
function Diagnostic () {
    if (ClientHelper(421, 0)) {
        ...
        serial.writeLine("VMAP to Real: N/A")
        console.error("VMAP TO REAL FAIL")
        return 0
    }
    fakepacket = [
    "192",
